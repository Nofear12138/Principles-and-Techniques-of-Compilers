...
int DecAnalyse(Node *cur, struct SymbolBucket *scope, Type type)
{
    ...
    */
    ...
    if (tmpnode == NULL)
    {
        ...
        if (res2 == 0  && queryType->kind == STRUCTURE)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        while (1)
                        {
                            ...
                            int tmpoffset = 0;
                            FieldList tmpDefField = DefStruct(tmpDefNode, structName,curoffset,&tmpoffset);
                            curoffset+=tmpoffset;
                            if (res == NULL)
                            {
                                res = tmpDefField;
...
FieldList DefStruct(Node *cur, char *structname,int curoffset,int *tmpoffset)
{
    ...
    */
    int offset = 0;
    Node *tmp111 = GetChild(tmpdeclist,1);
    while (GetChild(tmpdeclist, 1) != NULL)
    {
        ...
        if (queryStructname(DecName) == 0)
        {
            ...
            printf("Error 15 No.1\n");
        }
        else
        {
            InsertStruct(tmpdecf->type, DecName,offset+curoffset,structname);
            //printf("offset = %d\n",offset);
        }

        int typesize = GetSize(tmpdecf->type);
        offset+=typesize;
